...
namespace strutils
{
	...
	std::string get_parent_path(std::string path, bool lower)
	{
		path = path_normalize(path);
		size_t pos = path.find_last_of("/");
		return pos != std::string::npos && pos < path.size() - 1 ?
			lower ? str_to_lower(path.substr(0, pos)) : path.substr(0, pos) :
			"";
	}

	std::string get_filename(std::string path, bool lower)
	{
		path = path_normalize(path);
		size_t pos = path.find_last_of("/");
		return pos != std::string::npos && pos < path.size() - 1 ?
