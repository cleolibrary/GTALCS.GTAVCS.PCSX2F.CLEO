...
EE_LIBS += %s
EE_LDFLAGS = -Wl,--entry=init -Wl,--no-relax -Wl,-Map,../data/%s.map -Wl,'--defsym=BASE_ADDRESS=$(BASE_ADDRESS)'
...
include $(PS2SDK)/samples/Makefile.eeglobal
      ...
      file:write(string.format(str, scripts_addr .. prj_name, files, base, libs, scripts_addr .. prj_name))
      file:close()
   end
end

function writelinkfile_ps2()
   file = io.open("source/linkfile", "w")
   if (file) then
str = [[
ENTRY(__start);
